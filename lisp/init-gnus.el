...
(setq gnus-group-name-charset-group-alist '((".*" . gbk)))
  ...
  (concat (car (cdr gnus-tmp-method)) ":"
		  (or (gnus-group-find-parameter gnus-tmp-group 'display-name)
			  (let ((prefix (assq 'remove-prefix (cddr gnus-tmp-method))))
				(if (and prefix
						 (string-match
						  (concat "^\\(" (regexp-quote (cadr prefix)) "\\)")
...
;; .authinfo.gpg for gmail accounts:
;; machine shortname1 login address1@gmail password yourpasswd1 port 993 force yes
;; machine shortname2 login address2@gmail password yourpasswd2 port 993 force yes
