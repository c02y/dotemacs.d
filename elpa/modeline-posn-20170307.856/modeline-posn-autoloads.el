...

;;;### (autoloads nil "modeline-posn" "modeline-posn.el" (22726 37018
;;;;;;  734768 280000))
...
(defvar modelinepos-style '((if modelinepos-rect-p " %d rows, %d cols" " %d ch, %d l") (if modelinepos-rect-p (count-lines (region-beginning) (region-end)) (abs (- (mark t) (point)))) (if modelinepos-rect-p (if (fboundp 'rectangle--pos-cols) (let ((rpc (save-excursion (rectangle--pos-cols (region-beginning) (region-end))))) (abs (- (car rpc) (cdr rpc)))) (let ((start (region-beginning)) (end (region-end)) startcol endcol) (save-excursion (goto-char start) (setq startcol (current-column)) (beginning-of-line) (goto-char end) (setq endcol (current-column)) (when (< endcol startcol) (let ((col startcol)) (setq startcol endcol endcol col))) (abs (- startcol endcol))))) (count-lines (mark t) (point)))) "\
